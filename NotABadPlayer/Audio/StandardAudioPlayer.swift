...
class StandardAudioPlayer : NSObject, AudioPlayer {
    ...
    var playOrder: AudioPlayOrder {
        ...
        set {
            ...
            onPlayOrderChange(order: newValue)
        }
    }
    ...
    var playerHistory: AudioPlayerHistory
    private var remote: AudioPlayerRemote
    
    public init(_ remote: AudioPlayerRemote?=nil) {
        synchronous = DispatchQueue(label: "AudioPlayer.synchronous")
        playSync = DispatchQueue(label: "AudioPlayer.synchronous.play")
        self.playerHistory = AudioPlayerHistoryStandard()
        self.remote = AudioPlayerRemote()
        super.init()
        self.playerHistory = AudioPlayerHistoryStandard(player: self)
    ...
    func playNextBasedOnPlayOrder() throws {
        ...
        playlist.goToTrackBasedOnPlayOrder(playOrder: self.playOrder)
...
// # AudioPlayerObserving
extension StandardAudioPlayer: AudioPlayerObserving {
...
extension StandardAudioPlayer : AVAudioPlayerDelegate {
